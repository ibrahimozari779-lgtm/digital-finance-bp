...
def build_cash_bridge(current: dict[str,Any], previous: dict[str,Any] | None = None) -> dict[str,Any]:
    if previous is None:
        return {'available':False,'mode':'snapshot','note':'İki dönemli karşılaştırmalı nakit köprüsü için en az iki dönem mizanı gereklidir; tek dönemde cari işletme sermayesi dengesi listelenmiştir.','working_capital_proxy':{'receivables':_v(current,'receivables'),'inventory':_v(current,'inventory'),'payables':_v(current,'payables')}}
